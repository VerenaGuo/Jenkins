pipeline {
    ...
    environment {
        ...
        // 项目存储地址
        buildPath = "/Users/yuguo/.jenkins/workspace/Tencent-QA-Android/velo-android/"
    }
    stages {
        stage("Initial") {
            steps {
                ...
                dir("velo-android") {
                    ...
                    sh "cp -f /Users/yuguo/Documents/androidconfigjenkins/build1.gradle /Users/yuguo/.jenkins/workspace/Tencent-QA-Android/velo-android/build.gradle; cp -f /Users/yuguo/Documents/androidconfigjenkins/gradle1.properties  /Users/yuguo/.jenkins/workspace/Tencent-QA-Android/velo-android/gradle.properties;cp -f /Users/yuguo/Documents/androidconfigjenkins/build_mock.gradle /Users/yuguo/.jenkins/workspace/Tencent-QA-Android/velo-android/velo-adobe/vladobe/build.gradle; cp -f /Users/yuguo/Documents/androidconfigjenkins/network_security_config1.xml  /Users/yuguo/.jenkins/workspace/Tencent-QA-Android/velo-android/app/src/main/res-main/xml/network_security_config.xml"
                }
            }
        }
        stage("Build") {
        ...
        stage("Deploy") {
            steps {
                dir("velo-android") {
                    ...
                    sh "cp -f ${env.buildPath}app/build/outputs/apk/qaTencent/debug/app-qaTencent-debug.apk ${env.webBasePath}${env.Project_Name}/${env.date}/app-qaTencent-debug.apk"
                    // 生成二维码
                    sh "cd ${env.webBasePath}${env.Project_Name}/${env.date};/usr/local/bin/qrencode -o qrcode.png -s 4 -m 2 'http://192.168.8.56/android/${env.Project_Name}/${env.date}/app-qaTencent-debug.apk'"
                }
            }
        }
            ...
            steps {
                script {
                    ...
                    if (fileExists(imagePath)) {
                        ...
                        mail(
                            subject: 'Tencent QA-Android Velo',
                            body: """
                            <html><body>
                            <p>扫描二维码安装Tencent QA-Android Velo应用</p>
        ...
        success {
            ...
            // 使用优化后的 HTML 代码设置构建描述
                ...
                <div style="padding: 10px; background-color: #000; border: 1px solid #333; color: #fff;">
                    ...
                    <a href='http://192.168.8.56/android/${env.Project_Name}/${env.date}/app-qaTencent-debug.apk' 
            ...
            sh "cp -f ${env.webBasePath}${env.Project_Name}/${env.date}/qrcode.png /Library/WebServer/Documents/qrcodeandroid_tencent.png"
